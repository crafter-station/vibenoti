...
export type HoverRevealMode = "progressive" | "spotlight";
...
export interface EChartsAreaChartProps<TData extends Record<string, unknown>> {
  ...
  hoverRevealMode?: HoverRevealMode; // progressive colors up to the pointer; spotlight colors only its local segment
...
type OptionBuildContext = {
  ...
  hoverRevealMode: HoverRevealMode;
  revealIndex: number | null; // pointer's x-index while revealing
...
function buildAreaSeries(ctx: OptionBuildContext): LineSeriesOption[] {
  ...
  return areas.flatMap((area): LineSeriesOption[] => {
    ...
    const spotlight = reveal && hoverRevealMode === "spotlight";
    ...
    const mainValues: (number | null)[] = buffer
      ...
      : spotlight
        ? revealActive
          ? sliceSpotlight(values, revealIndex as number)
          : values.map(() => null)
        : revealActive
    ...
    // the real one. Progressive mode reveals it past the cursor; spotlight mode
    // keeps it visible at rest and colors only the local hovered segment.
    if (reveal) {
      ...
      const revealBase: LineSeriesOption = {
        ...
        data: spotlight
          ? values
          : revealActive
            ? sliceFrom(values, revealIndex as number)
            : values,
        ...
        lineStyle: {
          ...
          width: area.strokeWidth,
          type: mainDash,
          opacity: spotlight || revealActive ? 0.3 : 0,
        },
        emphasis: { disabled: true },
        blur: { lineStyle: { opacity: spotlight || revealActive ? 0.3 : 0 } },
        tooltip: { show: false },
      };
...
// Keep the hovered category and one neighbor so ECharts can draw a local line
// segment and area fill while the full muted trend remains visible underneath.
function sliceSpotlight<T>(vals: readonly T[], idx: number): (T | null)[] {
  const start = idx === vals.length - 1 ? Math.max(0, idx - 1) : idx;
  const end = Math.min(vals.length - 1, start + 1);
  return vals.map((value, index) => (index >= start && index <= end ? value : null));
}
...
export function EChartsAreaChart<TData extends Record<string, unknown>>({
  ...
  useEffect(() => {
    ...
    // Hover-reveal: progressively color up to the pointer, or spotlight only
    // the local segment while keeping the complete trend muted.
    ...
    const pushReveal = (idx: number | null) => {
      ...
      const on = idx !== null;
      const spotlight = live.handlers.hoverRevealMode === "spotlight";
      chart.setOption(
        {
          series: keys.flatMap((key) => [
            {
              ...
              data: spotlight
                ? on
                  ? sliceSpotlight(live.revealValues[key] ?? [], idx)
                  : (live.revealValues[key] ?? []).map(() => null)
                : on
            ...
            },
            {
              ...
              data: spotlight
                ...
                : on
                  ...
                  : (live.revealValues[key] ?? []),
              lineStyle: { opacity: spotlight || on ? 0.3 : 0 },
            },
          ]),
        },
